...
def main():
    ...
    # 计算平均起始点和结束点
    start_point, end_point = StartEnd_Caculate(LineStart_Point, LineEnd_Point,
                                               original_length)  #########这里需要有一个滤波，对一整条的起始线进行滤波，也对结束线进行滤波，可以用排序，再去除头尾部分数据
    ...
    global pixl_width2
    pixl_width2 = end_point - start_point

   #掐头去尾
    pixl_length = original_length*3
    x, y = np.ogrid[0:pixl_width2:1,0:pixl_length:3]
   #print(x,y)
    allpixl = pixl_width2*original_length
    z=np.array(range(allpixl)).reshape(pixl_width2,original_length)
    #print(allpixl)
   # print(z)
    #深度数据全部赋值为0
    for i in range(pixl_width2):
